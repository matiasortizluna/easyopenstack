<template>
  <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
    <div class="container mx-auto px-6 py-8">
      <h3 class="text-gray-700 text-3xl font-medium">Images <button class="btn btn-info" @click="toggleModal()">Add <i class="far fa-plus-square"></i></button></h3>
      <br />
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="input-group mb-3">
              <input type="text" class="form-control" placeholder="Image name" aria-label="Image name" aria-describedby="basic-addon1" required v-model="imageName">
            </div>
            <div class="input-group mb-3">
              <input type="text" class="form-control" placeholder="URI to download the image" aria-label="Image URI" aria-describedby="basic-addon1" required v-model="imageURI">
            </div>
            ...
            <div class="input-group mb-3">
              <input type="number" class="form-control" placeholder="Min. RAM (MB)" aria-label="Min. Disk" required v-model="minRam">
                <span class="input-group-text">|</span>
              <input type="number" class="form-control" placeholder="Min. Disk (GB)" aria-label="Min. Disk" required v-model="minDisk">
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
            <button type="button" @click="addImage()" class="btn btn-primary" >Add</button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    addImage(){  
      ...
      if (this.imageName && this.imageURI && this.minRam && this.minDisk){
        ...
        .catch((error) => {
          this.messageModal =""
          this.errorMessageModal = error.response.data.message+" (Check the URI)";
          console.log(error)
        })
